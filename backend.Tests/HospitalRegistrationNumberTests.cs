...
{
    /// <summary>
    /// Hospital registration-number uniqueness on registration and resubmission, and the empty-ID placeholder guard.
    ...
    public class HospitalRegistrationNumberTests
    {
        ...
        [Fact]
        public async Task Resubmission_Cannot_Take_Another_Hospitals_Number_But_Can_Keep_Or_Change_Its_Own()
        {
            var context = NewContext();
            var service = new HospitalService(context);
            await service.CreateHospitalAsync(Hospital("a@h.org", "PHSRC/PH/1234"));
            ...
            var ex = await Assert.ThrowsAsync<InvalidOperationException>(() =>
                service.ResubmitHospitalAsync(second.HospitalId, new ResubmitHospitalDto { RegistrationNumber = " phsrc/ph/1234" }));
            Assert.Equal(Duplicate, ex.Message);
            ...
            var kept = await service.ResubmitHospitalAsync(second.HospitalId, new ResubmitHospitalDto { RegistrationNumber = "phsrc/ph/5678" });
            Assert.Equal("PHSRC/PH/5678", kept.RegistrationNumber);

            var changed = await service.ResubmitHospitalAsync(second.HospitalId, new ResubmitHospitalDto { RegistrationNumber = " phsrc/ph/9999 " });
